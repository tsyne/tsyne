...
if (require.main === module) {
  const appInstance = app(resolveTransport(), { title: 'Settings' }, (a: App) => {
    const settings = new MockSettingsService();
  appInstance.setOnLastWindowClose(standaloneShutdownStrategy(appInstance));    createSettingsApp(a, settings);
  });
}
