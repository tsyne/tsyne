...
exports.ExpenseStore = void 0;
exports.buildExpenseTrackerApp = buildExpenseTrackerApp;
class ExpenseStore {
    constructor() {
        this.expenses = [
            {
    ...
    addExpense(amount, category, description, isRecurring = false, interval) {
        const expense = {
...
exports.ExpenseStore = ExpenseStore;
// ============================================================================
// VIEW BUILDER
// ============================================================================
function buildExpenseTrackerApp(a) {
    ...
    a.window({ title: 'Expense Tracker', width: 1000, height: 750 }, (win) => {
        win.setContent(() => {
            a.vbox(() => {
                // Header with summary
                a.hbox(() => {
                    ...
                    expensesContainer = a.vbox(() => {
                        ...
                        a.hbox(() => {
                            a.button('➕ Add Expense')
                                ...
                                if (result.submitted) {
                                    ...
                                    if (!isNaN(amount)) {
                                        store.addExpense(amount, result.values.category, result.values.description, result.values.recurring === 'true');
                                    }
                                }
                            });
                            a.spacer();
                        ...
                        a.vbox(() => {
                            ...
                            render: (expense) => {
                                ...
                                a.hbox(() => {
                                    a.label(`${icon} ${expense.category}`, 'expense-category').withId(`exp-category-${expense.id}`);
                                    a.vbox(() => {
                                        a.label(expense.description).withId(`exp-desc-${expense.id}`);
                                        a.label(`${expense.date.toLocaleDateString()}`, 'expense-date').withId(`exp-date-${expense.id}`);
                                    });
                                    a.spacer();
                                    a.label(`$${expense.amount.toFixed(2)}`, 'expense-amount').withId(`exp-amount-${expense.id}`);
                                    a.button('✕')
                                        .withId(`btn-delete-${expense.id}`)
                                        .onClick(() => store.deleteExpense(expense.id));
                                });
                            },
                            trackBy: (expense) => expense.id,
                        });
                    ...
                    budgetsContainer = a.vbox(() => {
                        ...
                        a.vbox(() => {
                            ...
                            render: (budget) => {
                                ...
                                const progressBar = '█'.repeat(Math.min(status.percentage, 100) / 10) +
                                ...
                                a.hbox(() => {
                                    a.vbox(() => {
                                        ...
                                        a.label(`$${status.spent.toFixed(2)} / $${status.limit.toFixed(2)} (${status.percentage}%)`).withId(`budget-stats-${budget.id}`);
                                    });
                                });
                            },
                            trackBy: (budget) => budget.id,
                        });
                    ...
                    analyticsContainer = a.vbox(() => {
                        ...
                        a.hbox(() => {
                            a.vbox(() => {
                                ...
                                a.label(`Total Spent: $${store.getTotalSpent().toFixed(2)}`).withId('analytics-total');
                                a.label(`This Month: $${store.getTotalSpentThisMonth().toFixed(2)}`).withId('analytics-month');
                                a.label(`Monthly Recurring: $${store.getMonthlyRecurringTotal().toFixed(2)}`).withId('analytics-recurring');
                            });
                            a.spacer();
                            a.vbox(() => {
                                ...
                                entries.slice(0, 5).forEach(([category, amount]) => {
                                    ...
                                    a.label(`${icon} ${category}: $${amount.toFixed(2)}`).withId(`analytics-cat-${category}`);
                                });
                            });
                        });
                        a.separator();
                        ...
                        a.vbox(() => {
                            ...
                            render: (expense) => {
                                const icon = store.getCategoryIcon(expense.category);
                                a.hbox(() => {
                                    a.label(`${icon} ${expense.category}: $${expense.amount.toFixed(2)}`).withId(`recurring-exp-${expense.id}`);
                                    a.label(`(${expense.recurringInterval})`).withId(`recurring-interval-${expense.id}`);
                                });
                            },
                            trackBy: (expense) => expense.id,
                        });
...
exports.default = buildExpenseTrackerApp;
