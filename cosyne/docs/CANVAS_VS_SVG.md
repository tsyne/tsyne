# CosyneContext vs SvgContext

Cosyne has two independent drawing systems that share the same Go bridge but are otherwise separate widget trees.

## API Model

| | **CosyneContext** (`cosyne/src/context.ts`) | **SvgContext** (`cosyne/src/svg/grammar.ts`) |
|---|---|---|
| **Vocabulary** | Canvas-native: `circle(x, y, r)`, `rect(x, y, w, h)` | SVG-native: `circle({ cx, cy, r })`, `rect({ x, y, width, height })` |
| **Parameters** | Positional args + options object | Single SVG-attributes object (mirrors SVG DOM) |
| **Grouping** | `ctx.group(x, y, fn)` / `ctx.transform({ translate, scale }, fn)` | `s.g({ transform: 'translate(...) scale(...)' }, fn)` — raw SVG transform strings |
| **Paths** | `CosynePath` with custom options | `s.path({ d: '...' })` with full SVG path normalizer (all commands to M/L/C/Z) |

## Architecture

| | **CosyneContext** | **SvgContext** |
|---|---|---|
...
| | **CosyneContext** | **SvgContext** |
|---|---|---|
| **Binding model** | Separate `BindingRegistry` — stores `() => T` functions per primitive, evaluated on `refreshBindings()` with deep-equality diffing | Inline on `SvgElement`: `.bindFill(() => color)`, `.bindPos(() => coords)`, `.bindOpacity(() => n)` |
...
- Animation system with easing, transitions, keyframes (built into SvgElement)
...
**Use SvgContext** when you want SVG semantics (viewBox, transform groups, path `d` strings), need to load/render `.svg` files, want D3-style data binding, or want to compose reusable SVG components via `s.g({ transform })`. The svg-clock and Big Ben demos are examples of this pattern.

## Relationship

Both systems call the same Tsyne bridge primitives (`canvasPath`, `canvasCircle`, etc.) underneath. SvgContext explicitly bypasses CosyneContext — they are parallel branches of the widget tree, not layers. The main `cosyne/src/index.ts` re-exports both via `export * from './svg'`.
