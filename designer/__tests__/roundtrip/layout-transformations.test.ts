...
app({ title: 'Test' }, (a) => {
  a.window({ title: 'Test' }, (win) => {
    win.setContent(() => {
      a.grid(3, () => {
        a.button('A', () => { console.log('A'); });
        a.button('B', () => { console.log('B'); });
        a.button('C', () => { console.log('C'); });
      });
    });
    win.show();
  });
  ...
  a.window({ title: 'Test' }, (win) => {
    win.setContent(() => {
      a.vbox(() => {
        ...
        a.button('Action', () => {}).withId('myButton');
      }).withId('container');
    });
    win.show();
  });
...
app({ title: 'Test' }, (a) => {
  a.window({ title: 'Test' }, (win) => {
    ...
    test('layout transformation preserves style classes', async () => {
      const original = `import { app, styles } from '../src';
